...
public class MarkerManager {
	...
	public void save() {
		this.markers.forEach(marker -> {
			this.configurationFile.getConfiguration().set("markers." + marker.getName() + ".world", marker.getLocation().getWorld().getName());
			this.configurationFile.getConfiguration().set("markers." + marker.getName() + ".x", marker.getLocation().getBlockX());
			this.configurationFile.getConfiguration().set("markers." + marker.getName() + ".y", marker.getLocation().getBlockY());
			this.configurationFile.getConfiguration().set("markers." + marker.getName() + ".z", marker.getLocation().getBlockZ());
		});

		this.configurationFile.getConfiguration().getConfigurationSection("markers").getKeys(false).forEach(marker -> {
			if (this.getMarker(marker) == null) {
				this.configurationFile.getConfiguration().getConfigurationSection("markers").set(marker, null);
			}
		});
